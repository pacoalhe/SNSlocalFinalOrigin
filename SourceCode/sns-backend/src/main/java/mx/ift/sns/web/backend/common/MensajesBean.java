...
/**
 * Proporciona los métodos para mostrar mensajes en la parte web. Actualiza los componentes p:message de PrimeFaces con
 * los textos indicados en formato 'Summary' o 'Detail'
 * @author 67059279
 */
public final class MensajesBean {

    /** Logger de la clase . */
    private static final Logger LOGGER = LoggerFactory.getLogger(MensajesBean.class);

    /**
     * Constructor privado.
     */
    private MensajesBean() {
    }
     ...
     * @param error error
     */
    public static void addErrorMsg(final String contenedorMsg, final Errores error) {
        String descripcion = Errores.getDescripcionError(error);
    ...
    /**
     * Método que inserta en un p:growl determinado, una descripcion.
     * @param contenedorMsg donde lo muestra si es un null al global
     * @param msg mensaje a mostrar.
     */
    public static void notificacionPendiente(final String contenedorMsg, String msg) {
        FacesMessage message = new FacesMessage(FacesMessage.SEVERITY_INFO, msg, "");
        FacesContext.getCurrentInstance().addMessage(contenedorMsg, message);

        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug("'{}' '{}'", contenedorMsg, contenedorMsg);
        }
    }
}
