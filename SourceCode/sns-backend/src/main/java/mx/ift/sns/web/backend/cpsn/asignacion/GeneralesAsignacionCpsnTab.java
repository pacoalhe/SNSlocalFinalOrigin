...
/** Clase de soporte para la pestaña de 'Generales' de Solicitudes de Asignación CPSN. */
public class GeneralesAsignacionCpsnTab extends TabWizard {

    /** Serial UID. */
    private static final long serialVersionUID = 1L;

    /** Logger de la clase. */
    private static final Logger LOGGER = LoggerFactory.getLogger(GeneralesAsignacionCpsnTab.class);
     ...
     * @param facade ICodigoCPSNFacade
     */
    public GeneralesAsignacionCpsnTab(Wizard pWizard, ICodigoCPSNFacade facade) {
    ...
    public boolean isAvanzar() {
        ...
        if (isTabHabilitado()) {
            ...
            // No se puede tocar el tab, dejamos pasar a la siguiente pestaña.
            r = true;
        }
        return r;
    }
    ...
    public void actualizaCampos() {
        try {
            ...
            if (solicitud.getProveedorSolicitante() != null) {
                listaRepresentantes = cpsnFacade.getRepresentantesLegales(
                        TipoContacto.REPRESENTANTE_LEGAL, solicitud.getProveedorSolicitante().getId());
                listaSuplentes = cpsnFacade.getRepresentantesLegales(
            ...
            if (solicitud.getNumeracionAsignadas() != null) {
                solicitudIniciada = (!solicitud.getNumeracionAsignadas().isEmpty());
            }
     ...
     * @return True si el botón ha de habilitarse.
     */
    public boolean isSalvarHabilitado() {
        if (this.isIniciado()) {
            return (solicitud.getProveedorSolicitante() != null
                    && solicitud.getFechaSolicitud() != null
                    && solicitud.getRepresentanteLegal() != null
                    && (!StringUtils.isEmpty(this.solicitud.getReferencia())));
        } else {
            return false;
        }
    }
        ...
        try {
            if (validaGeneral()) {
                ...
                if (solicitud.getTipoSolicitud() == null) {
                    ...
                    tipoSol.setCdg(TipoSolicitud.ASIGNACION_CPSN);
        ...
        if (solicitud.getFechaSolicitud() != null) {
            if (solicitud.getFechaSolicitud().after(fechaHoy)) {
                MensajesBean.addErrorMsg(MSG_ID,
                        MensajesBean.getTextoResource("manual.generales.error.valida"),
    ...
    /**
     * Identificador de los representantes.
    ...
    /**
     * Identificador de los representantes suplentes.
     * @return List<Contacto>
     */
