 ...
 */
public class SolicitudLiberacionNgLazyModel extends LazyDataModel<SolicitudLiberacionNg> {

    /** Serialización. */
    private static final long serialVersionUID = 1L;

    /** Logger de la clase. */
    private static final Logger LOGGER = LoggerFactory.getLogger(SolicitudLiberacionNgLazyModel.class);

    /** Colección de objetos. */
    private List<SolicitudLiberacionNg> dataSource;
    ...
    /** Constructor. */
    public SolicitudLiberacionNgLazyModel() {
        dataSource = new ArrayList<SolicitudLiberacionNg>(1);
    }

    @Override
    public SolicitudLiberacionNg getRowData(String rowKey) {
        for (SolicitudLiberacionNg solicitud : dataSource) {
    ...
    @Override
    public Object getRowKey(SolicitudLiberacionNg pSolicitud) {
        return pSolicitud.getId().toString();
    }

    @Override
    public List<SolicitudLiberacionNg> load(int first, int pageSize, String sortField, SortOrder sortOrder,
        ...
        try {
            ...
            dataSource.clear();
            dataSource.addAll(service.findAllSolicitudesLiberacion(filtros));
