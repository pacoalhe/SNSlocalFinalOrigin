...
public class CesionNgDerechosTab extends TabWizard {
     ...
     */
    public CesionNgDerechosTab(Wizard pWizard, INumeracionGeograficaService pNgService) {
    ...
    public void cargaValoresIniciales() {
        try {
            ...
            listaProveedoresCedentes = ngService.findAllProveedoresActivos();
    ...
    public void seleccionPstCedente() {
        try {
            ...
            if (solicitud.getProveedorSolicitante() != null) {
                listaRepresentantesLegalesPstCedente = ngService.getRepresentantesLegales(
                        TipoContacto.REPRESENTANTE_LEGAL, solicitud.getProveedorSolicitante().getId());
                listaRepresentantesSuplentesPstCedente = ngService.getRepresentantesLegales(
                        TipoContacto.OTROS_REPRESENTANTES_LEGALES, solicitud.getProveedorSolicitante().getId());

                // Proveedores Cesionarios Disponibles
                listaProveedoresCesionarios = ngService.findAllCesionarios(solicitud.getProveedorSolicitante());
    ...
    public void seleccionPstCesionario() {
        try {
            if (solicitud.getProveedorCesionario() != null) {
                listaRepresentantesLegalesPstCesionario = ngService.getRepresentantesLegales(
                        TipoContacto.REPRESENTANTE_LEGAL, solicitud.getProveedorCesionario().getId());
                listaRepresentantesSuplentesPstCesionario = ngService.getRepresentantesLegales(
    ...
    private boolean isValido() {
        try {
            ...
            if (solicitud.getProveedorCesionario().getTipoProveedor().getCdg().equals(TipoProveedor.CONCESIONARIO)) {
                // El Cesionario Concesionario ha de tener IDO
                if (solicitud.getProveedorCesionario().getIdo() == null) {
                    MensajesBean.addErrorMsg(MSG_ID, "El Cesionario Concesionario ha de tener IDO.", "");
                    return false;
                }
            }
