...
public class CesionNgNumeracionTab extends TabWizard {

    /** Logger de la clase. */
    private static final Logger LOGGER = LoggerFactory.getLogger(CesionNgNumeracionTab.class);
    ...
    private CesionSolicitadaNg cesSolSeleccionada;
     ...
     */
    public CesionNgNumeracionTab(Wizard pWizard, INumeracionGeograficaService pNgService) {
    ...
    public void cargaValoresIniciales() {
        try {
            ...
            listaTiposModalidad = new ArrayList<>();
            ...
            pstComercializadora = ngService.getTipoProveedorByCdg(TipoProveedor.COMERCIALIZADORA);
            pstConcesionario = ngService.getTipoProveedorByCdg(TipoProveedor.CONCESIONARIO);
    ...
    public void resetTab() {
        if (this.isIniciado()) {
            ...
            usarIdoCesionario = false;
            habilitarUsarIdo = false;
    ...
    public void realizarBusqueda() {
        try {
            ...
            PaginatorUtil.resetPaginacion("FORM_SolicitudCesion:TAB_Numeracion:TBL_Series",
     ...
     */
    private boolean aplicarCesiones() throws Exception {
        try {
            if (validadRangosACeder()) {
                // Aplicamos las cesiones solicitadas y reservamos los rangos si es necesario.
                SolicitudCesionNg solCes = ngService.applyCesionesSolicitadas(solicitud);
            ...
            // Si se produce error puede que alguna cesiones hayan sido ya aplicadas. Recuperamos la solicitud para
            // mostrarlas.
            solicitud = ngService.getSolicitudCesionEagerLoad(solicitud);
        ...
        if (this.isTabHabilitado()) {
            ...
            if (listaCesiones.isEmpty()) {
                ...
                try {
                    ...
                    // Error procesando error al aplicar las cesiones
    ...
    public void actualizaCampos() {
        try {
            // Limpiamos los datos de búsqueda por si se ha retrocedido desde la página de cesión y se vuelve a ella.
            this.limpiarBusqueda();
            tipoProveedorSeleccionado = null;
            ...
            if (!solicitud.getEstadoSolicitud().getCodigo().equals(EstadoSolicitud.SOLICITUD_CANCELADA)) {
                ...
                for (CesionSolicitadaNg cesSol : listaCesiones) {
                    existeRango = true;
                    ...
                    if (!existeRango) {
                        ...
                        sbAviso.append(", Nir: ").append(cesSol.getCdgNir());
    ...
    public void agregarCesion() {
        try {
            ...
            // Si no se utiliza el Ido del Cesionario hay que seleccionar un Convenio
            if (!isUsarIdoCesionario() && convenioConcesionarioSeleccionado == null) {
                MensajesBean.addErrorMsg(MSG_ID, "Es necesario seleccionar un convenio o el IDO del Cesionario", "");
                todoOk = false;
            }

            // El Cesionario seleccionado de convenio ha de tener IDO
            if (!isUsarIdoCesionario() && convenioConcesionarioSeleccionado != null) {
                if (convenioConcesionarioSeleccionado.getProveedorConcesionario().getIdo() == null) {
                    MensajesBean.addErrorMsg(MSG_ID,
                            "El Proveedor Concesionario del convenio seleccionado debe tener IDO.", "");
                    todoOk = false;
                }
            }
    ...
    private void cederFracciones() throws Exception {
        ...
        for (RangoSerie fraccion : listaRangoFraccionados) {
            ...
            if (fraccion.getEstadoRango().getCodigo().equals(EstadoRango.AFECTADO)) {
                RangoSerie rangoOriginal = (RangoSerie) RangosSeriesUtil.getRangoInicial(
                        fraccion, listaRangosFracciones);
                CesionSolicitadaNg cs = getCesionSolicitadaFromRango(rangoOriginal);
    ...
    private void cederRangosSeleccionados() throws Exception {
        if (validarCesionSinFraccionamiento()) {
            for (RangoSerie rango : multiSelectionManager.getRegistrosSeleccionados()) {
                CesionSolicitadaNg cs = getCesionSolicitadaFromRango(rango);
                listaCesiones.add(cs);
            }
        }
    }
    ...
    public void eliminarCesion() {
        try {
            PeticionCancelacion checkCancelacion = ngService.cancelCesion(cesSolSeleccionada, true);
            ...
            // Si ha cesiones seleccionadas deshabilitamos la fusión.
            // fusionHabilitado = (listaCesiones.isEmpty());
        } catch (RegistroModificadoException rme) {
            MensajesBean.addErrorMsg(MSG_ID, Errores.ERROR_0015);
        } catch (Exception e) {
            LOGGER.error("Error inesperado", e);
    ...
    private boolean validarCesionSinFraccionamiento() throws Exception {
        ...
        ArrayList<RangoSerie> rangosNoSoportados = new ArrayList<RangoSerie>();
        for (RangoSerie rango : multiSelectionManager.getRegistrosSeleccionados()) {
            ...
            // Comprueba que el tipo de red de la numeracion del rango que se quiere ceder sea soportada
            // por el cesionario
            String tipoRedNumeracion = rango.getTipoRed().getCdg();
            String tipoRedPst = solicitud.getProveedorCesionario().getTipoRed().getCdg();
        ...
        // Solo es posible ceder rangos libres
        if (!rangosNoSoportados.isEmpty()) {
            StringBuffer sb = new StringBuffer();
            sb.append("No es posible agregar los rangos seleccionados. ");
            sb.append("El Proveedor Cesionario no soporta el tipo de red de los siguientes rangos:<br>");
            for (RangoSerie rango : rangosNoSoportados) {
                sb.append("Serie: ").append(rango.getId().getSna()).append(", Num.Inicio: ");
                sb.append(rango.getNumInicio()).append(", Tipo Red: ");
                sb.append(rango.getTipoRed().getDescripcion());
                sb.append("<br>");
            }
            MensajesBean.addErrorMsg(MSG_ID, sb.toString(), "");
            return false;
    ...
    private boolean validarCesionConFraccionamiento() throws Exception {
        ...
        // Comprueba que el tipo de red de la numeracion del rango que se quiere ceder sea soportada
        // por el cesionario
        String tipoRedNumeracion = rangoSeleccionado.getTipoRed().getCdg();
        String tipoRedPst = solicitud.getProveedorCesionario().getTipoRed().getCdg();
        ...
        if (!tipoRedNumeracion.equals(TipoRed.AMBAS)) {
            ...
            if (!tipoRedPst.equals(TipoRed.AMBAS)) {
                if (!tipoRedNumeracion.equals(tipoRedPst)) {
                    StringBuffer sb = new StringBuffer();
                    sb.append("El Proveedor Cesionario no soporta el tipo de red del rango seleccionado:<br>");
                    sb.append("Serie: ").append(rangoSeleccionado.getId().getSna()).append(", Num.Inicio: ");
                    sb.append(rangoSeleccionado.getNumInicio()).append(", Tipo Red: ");
                    sb.append(rangoSeleccionado.getTipoRed().getDescripcion());
                    MensajesBean.addErrorMsg(MSG_ID, sb.toString(), "");
                    return false;
                }
            }
    ...
    private boolean validarCesionExistente() throws Exception {
        List<RangoSerie> rangosEnCesion = new ArrayList<RangoSerie>();
        if (usarFraccionamiento) {
            CesionSolicitadaNg cesSolTemp = this.getCesionSolicitadaFromRango(rangoSeleccionado);
            if (arrayContains(listaCesiones, cesSolTemp)) {
                rangosEnCesion.add(rangoSeleccionado);
            }
        } else {
            for (RangoSerie rango : multiSelectionManager.getRegistrosSeleccionados()) {
                CesionSolicitadaNg cesSolTemp = this.getCesionSolicitadaFromRango(rango);
                if (arrayContains(listaCesiones, cesSolTemp)) {
        ...
        if (!rangosEnCesion.isEmpty()) {
            ...
            for (RangoSerie rango : rangosEnCesion) {
    ...
    private boolean validadRangosACeder() throws Exception {
        // Antes de aplicar las cesiones comprobamos que todos los rangos sigan
        // con status 'Asignado'
            ...
            if (cesSol.getEstado().getCodigo().equals(EstadoCesionSolicitada.PENDIENTE)) {
                if (cesSol.getFraccionamientoRango().equals("N")) {
                    if (ngService.existeRangoSerie(
                            cesSol.getIdNir(), cesSol.getSna(), cesSol.getNumInicio(),
                            cesSol.getProveedorCedente())) {
    ...
    /**
     * Crea un objeto CesionSolicitadaNg en base a la información de un Rango.
     * @param pRango Rango con la información de la cesión.
     * @return CesionSolicitadaNg
     * @throws Exception en caso de error.
     */
    private CesionSolicitadaNg getCesionSolicitadaFromRango(RangoSerie pRango) throws Exception {
        CesionSolicitadaNg cs = new CesionSolicitadaNg();

        // Cesión Solicitada
        cs.setId(null); // Hasta que no se guarda no se obtiene un Id
        cs.setIdNir(pRango.getSerie().getNir().getId());
        cs.setCdgNir(pRango.getSerie().getNir().getCodigo());
        cs.setIdAbn(pRango.getSerie().getNir().getAbn().getCodigoAbn());
        cs.setSna(pRango.getSerie().getId().getSna());
        cs.setIdSerieInicial(pRango.getSerie().getId().getSna());
        cs.setCentralDestino(pRango.getCentralDestino());
        cs.setCentralOrigen(pRango.getCentralOrigen());
        cs.setNumInicio(pRango.getNumInicio().toString());
        cs.setNumFinal(pRango.getNumFinal().toString());
        cs.setPoblacion(pRango.getPoblacion());
        cs.setSolicitudCesion(solicitud);
        cs.setTipoModalidad(pRango.getTipoModalidad());
        cs.setTipoRed(pRango.getTipoRed());
        ...
        cs.setIdoPnn(pRango.getIdoPnn());
        cs.setIdaPnn(pRango.getIdaPnn());

        // Información de la solicitud de asignación.
        cs.setNumOficioRango(pRango.getOficioAsignacion());
    ...
    /**
     * Indica si una lista contiene una CesionSolicitadaNg sin usar los métos Equals de Object.
     * @param pList Lista de objetos CesionSolicitadaNg
     * @param pCesSol CesionSolicitadaNg a comparar
     * @return True si la CesionSolicitadaNg esta en la lista
     * @throws Exception en caso de error
     */
    private boolean arrayContains(List<CesionSolicitadaNg> pList, CesionSolicitadaNg pCesSol) throws Exception {

        // Cuando se guardan los cambios las CesionSolicitadaNg tienen otra instancia diferente dada
        ...
        for (CesionSolicitadaNg cs : pList) {
            if (cs.getIdNir().equals(pCesSol.getIdNir())
                    && cs.getSna().equals(pCesSol.getSna())
    ...
    private boolean guardarCambios() {
        try {
            ...
            ArrayList<CesionSolicitadaNg> cesSolEliminar = new ArrayList<CesionSolicitadaNg>();
            for (CesionSolicitadaNg cesSol : solicitud.getCesionesSolicitadas()) {
            ...
            for (CesionSolicitadaNg cesSol : cesSolEliminar) {
            ...
            for (CesionSolicitadaNg cesSol : listaCesiones) {
                if (!arrayContains(solicitud.getCesionesSolicitadas(), cesSol)) {
                    if (isUsarIdoCesionario()) {
                        cesSol.setUsarIdoCesionario("S");
                        cesSol.setUsarConvenioConcesionario("N");
                        cesSol.setConvenioCesionario(null);
                    } else {
                        cesSol.setUsarIdoCesionario("N");
                        cesSol.setUsarConvenioConcesionario("S");
            ...
            solicitud = ngService.saveSolicitudCesion(solicitud);
     ...
     * @return List
     */
    public List<CesionSolicitadaNg> getListaCesiones() {
     ...
     * @return boolean
     */
    public boolean isUsarIdoCesionario() {
        return usarIdoCesionario;
    }

    /**
     * Indica si se ha de utilizar el IDO del Cesionario Concesionario para enrutar la numeración que recibe.
     * @param usarIdoCesionario boolean
     */
    public void setUsarIdoCesionario(boolean usarIdoCesionario) {
     ...
     * @return boolean
     */
    public boolean isHabilitarUsarIdo() {
        return habilitarUsarIdo;
    }

    /**
     * Indica si el Proveedor Cesionario es Concesionario o Ambos y se puede utilizar su IDO.
     * @param habilitarUsarIdo boolean
     */
    public void setHabilitarUsarIdo(boolean habilitarUsarIdo) {
        this.habilitarUsarIdo = habilitarUsarIdo;
    }
     ...
     * @return CesionSolicitadaNg
     */
    public CesionSolicitadaNg getCesSolSeleccionada() {
        return cesSolSeleccionada;
    }

    /**
     * Cesion Solicitada seleccionada de la tabla de Cesiones.
     * @param cesSolSeleccionada CesionSolicitadaNg
     */
    public void setCesSolSeleccionada(CesionSolicitadaNg cesSolSeleccionada) {
