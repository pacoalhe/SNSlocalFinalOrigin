...
public final class ValidarFicheroEquipos {
     ...
     * @return EquiposCpsn lista de equipos.
    ...
    public static CargaEquiposExcel extraerExcel(File fichero) throws Exception {
        ...
        try {
            ...
            while (rows.hasNext()) {
                ...
                EquipoCpsnExcel data = new EquipoCpsnExcel();
                while (cells.hasNext()) {
     ...
     * @param codigoCPSNDao dao de codigos nacionales
     * @return listado de errores
     */
    public static CargaEquiposExcel validarEquipos(CargaEquiposExcel sheetData, Proveedor pst,
            IAbnDao abnDao, IPoblacionDao poblacionDao, ICodigoCPSNDao codigoCPSNDao) {

        for (EquipoCpsnExcel equipo : sheetData.getEquiposExcel()) {
            if (validarDuplicados(sheetData.getEquiposExcelValidos(), equipo)) {
                procesaRegistro(equipo, pst, abnDao, poblacionDao, codigoCPSNDao);
            } else {
                equipo.addErrorValidacion(
                        new ErrorRegistro(equipo.getColumnas().get(EquipoCpsnExcel.POS_REGISTRO),
                                ErrorRegistro.TIPO_AVISO_ERROR, 0, equipo.getColumnas()
                                        .get(EquipoCpsnExcel.POS_CPS), "Equipo duplicado."));
     ...
     */
    private static EquipoCpsnExcel procesaRegistro(EquipoCpsnExcel equipo, Proveedor pst,
            IAbnDao abnDao, IPoblacionDao poblacionDao, ICodigoCPSNDao codigoCPSNDao) {
        String numEquipo = null;

        String cps = NumerosUtils.decimalABinario(equipo.getColumnas().get(EquipoCpsnExcel.POS_CPS));
        ...
        for (int i = 0; i < textosCabecera[0].length; i++) {
            ...
            switch (i) {
            case EquipoCpsnExcel.POS_REGISTRO:
     ...
     * @param abnDao dao de abns
     */
    private static void validarAbn(int columna, String cps, EquipoCpsnExcel registro, IAbnDao abnDao) {
        String abn = registro.getColumnas().get(columna);
        // Se valida que el abn es obligatorio.
        if (abn == null || abn.isEmpty()) {
            registro.addErrorValidacion(new ErrorRegistro(registro.getColumnas().get(EquipoCpsnExcel.POS_REGISTRO),
     ...
     * @param registro recibido
     */
    private static void validarPoblacion(int columna, String cps, EquipoCpsnExcel registro) {
     ...
     * @param registro recibido
     */
    private static void validarMunicipio(int columna, String cps, EquipoCpsnExcel registro) {
     ...
     * @param registro recibido
     */
    private static void validarEstado(int columna, String cps, EquipoCpsnExcel registro) {
     ...
     * @param registro recibido
     */
    private static void validarLongitud(int columna, String cps, EquipoCpsnExcel registro) {
     ...
     * @param registro recibido
     */
    private static void validarLatitud(int columna, String cps, EquipoCpsnExcel registro) {
        ...
        if (latitud == null || latitud.isEmpty()) {
            registro.addErrorValidacion(new ErrorRegistro(registro.getColumnas().get(EquipoCpsnExcel.POS_REGISTRO),
                    ErrorRegistro.TIPO_AVISO_ERROR,
                    columna, cps,
                    "Latitud vacía o nula. Se procesa como 0° 0' 0\""));
        }
     ...
     * @param poblacionDao dao de poblaciones
     */
    private static void validarOrganizacionTerritorial(String cps, EquipoCpsnExcel registro,
            IPoblacionDao poblacionDao) {
        String abn = registro.getColumnas().get(EquipoCpsnExcel.POS_ABN);
        String poblacion = registro.getColumnas().get(EquipoCpsnExcel.POS_COD_LOCALIDAD);
        String municipio = registro.getColumnas().get(EquipoCpsnExcel.POS_COD_MUNICIPIO);
        String estado = registro.getColumnas().get(EquipoCpsnExcel.POS_COD_ESTADO);
     ...
     * @param registro recibido
     */
    private static void validarNombre(int columna, String cps, EquipoCpsnExcel registro) {
     ...
     * @param registro recibido
     */
    private static void validarClave(int columna, String cps, EquipoCpsnExcel registro) {
     ...
     * @param registro recibido
     */
    private static void validarTipo(int columna, String cps, EquipoCpsnExcel registro) {
     ...
     * @param registro recibido
     */
    private static void validarTipoCps(int columna, String cps, EquipoCpsnExcel registro) {
        String tipo = registro.getColumnas().get(columna);
        ...
        if (tipo == null || tipo.isEmpty()) {
            ...
            if (!"0".equals(tipo) && !"N".equals(tipo)) {
                registro.addErrorValidacion(new ErrorRegistro(registro.getColumnas().get(EquipoCpsnExcel.POS_REGISTRO),
                        ErrorRegistro.TIPO_AVISO_ERROR,
                        columna, cps,
                        "Tipo de CPS del equipo inválido. Sólo se importarán los equipos nacionales (N)."));
            }
        }
    }
     ...
     * @param codigoCPSNDao dao de codigos nacionales
     */
    private static void validarCps(int columna, String cps, EquipoCpsnExcel registro, Proveedor pst,
            ICodigoCPSNDao codigoCPSNDao) {
        ...
        if (cpsi == null || cpsi.isEmpty()) {
            ...
            if (!NumberUtils.isNumber(cpsi.trim())) {
                ...
                if (CPS_MIN > Integer.parseInt(cpsi) || Integer.parseInt(cpsi) > CPS_MAX) {
                    ...
                    if (!cpsAsignadoAPst(Integer.parseInt(cpsi), pst, codigoCPSNDao)) {
                        registro.addErrorValidacion(new ErrorRegistro(registro.getColumnas().get(
                                EquipoCpsnExcel.POS_REGISTRO),
                                ErrorRegistro.TIPO_AVISO_ERROR,
                                columna, cps,
     ...
     */
    public static boolean cpsAsignadoAPst(Integer cps, Proveedor pst, ICodigoCPSNDao codigoCPSNDao) {
     ...
     */
    private static boolean validarDuplicados(List<EquipoCpsnExcel> equiposValidos, EquipoCpsnExcel equipo) {

        if (equiposValidos != null) {
            String tipoEquipo = equipo.getColumnas().get(EquipoCpsnExcel.POS_TIPO_CPS);
            tipoEquipo = (tipoEquipo.equals("0") || tipoEquipo.equals("N")) ? "N" : "I";
            for (EquipoCpsnExcel equip : equiposValidos) {
                if (equip.getColumnas().get(EquipoCpsnExcel.POS_CPS)
                        .equals(equipo.getColumnas().get(EquipoCpsnExcel.POS_CPS))
                        && equip.getColumnas().get(EquipoCpsnExcel.POS_LONGITUD)
                                .equals(equipo.getColumnas().get(EquipoCpsnExcel.POS_LONGITUD))
                        && equip.getColumnas().get(EquipoCpsnExcel.POS_LATITUD)
                                .equals(equipo.getColumnas().get(EquipoCpsnExcel.POS_LATITUD))
                        && equip.getColumnas().get(EquipoCpsnExcel.POS_NOMBRE)
                                .equals(equipo.getColumnas().get(EquipoCpsnExcel.POS_NOMBRE))) {

                    String tipo = equip.getColumnas().get(EquipoCpsnExcel.POS_TIPO_CPS);
                    tipo = (tipo.equals("0") || tipo.equals("N")) ? "N" : "I";
