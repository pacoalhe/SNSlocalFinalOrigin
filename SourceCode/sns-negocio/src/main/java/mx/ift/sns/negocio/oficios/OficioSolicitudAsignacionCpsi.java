...
/** Proporciona los métodos para la generación del oficio de Solicitud de Asignación de CPSI. */
public class OficioSolicitudAsignacionCpsi extends GeneradorOficio {

    /** Logger de la clase. */
    private static final Logger LOGGER = LoggerFactory.getLogger(OficioSolicitudAsignacionCpsi.class);
     ...
     */
    public OficioSolicitudAsignacionCpsi(ParametrosOficio pParametros) {
        solicitud = (SolicitudAsignacionCpsi) pParametros.getSolicitud();
    ...
    public byte[] getDocumentoOficio(byte[] pPlantilla) throws Exception {
        ...
        case TipoDestinatario.PST_SOLICITANTE:
            ...
            tablas.put(GeneradorOficio.EXCEL_ASIGANCION_CPSI, getTablaAsignacionesCPSI());
            break;
        default:
            break;
        }
    ...
    /**
     * Crea la tabla de Información de Asginaciones de CPSI en formato Word (.docx). usando la librería Apache POI:
     * @return XWPFTable Tabla Word.
     */
    private XWPFTable getTablaAsignacionesCPSI() {
        // Cabecera
        String[] headerText = {"BINARIO", "DECIMAL", "FORMATO DECIMAL"};

        // Tamaño de tabla
        int tableSize = 1; // Cabecera
        for (CpsiAsignado cpsiAsig : solicitud.getCpsiAsignados()) {
            if (!cpsiAsig.getEstatus().getCodigo().equals(EstatusAsignacionCps.CANCELADO)) {
                tableSize++;
            }
        }
        ...
        try {
            ...
            for (CpsiAsignado cpsiAsig : solicitud.getCpsiAsignados()) {
                if (!cpsiAsig.getEstatus().getCodigo().equals(EstatusAsignacionCps.CANCELADO)) {
                    // Binario
                    generaCelda(table.getRow(fila).getCell(0), cpsiAsig.getBinario(), 10, false);

                    // Decimal
                    generaCelda(table.getRow(fila).getCell(1), String.valueOf(cpsiAsig.getIdCpsi()), 10, false);

                    // Formato Decimal
                    generaCelda(table.getRow(fila).getCell(2), cpsiAsig.getFormatoDecimal(), 10, false);
