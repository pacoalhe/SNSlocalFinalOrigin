...
import mx.ift.sns.negocio.utils.csv.ValidadorArchivoEliminadosPMN;
...
public class PlanMaestroService implements IPlanMaestroService {
	...
	try {
	    ...
	    // importamos portados
	    // ValidadorArchivoAgregadosPMN importAgregados = new
	    // ValidadorArchivoAgregadosPMN();

	    LOGGER.info("<--Inicia el procesamiento del csv de registros nuevos o modificados del PMN {}",
		    FechasUtils.getActualDate());

	    // Cargar BD
	    // importAgregados.validar(tmpAdded.getAbsolutePath());

	    LOGGER.info("<--finaliza el procesamiento del csv de registros nuevos o modificados del PMN {}",
		    FechasUtils.getActualDate());
	    ...
	    /*
	     * procesarPlanCSV(tmpAdded); cancelarPlanCSV(tmpDeleted);
	     */
	    ...
	    /*
	     * if (tmpAdded != null) { FileUtils.deleteQuietly(tmpAdded); }
	     * 
	     * if (tmpDeleted != null) { FileUtils.deleteQuietly(tmpDeleted); }
	     */
	...
	try {
		...
		if (datos.length > 1) {
			...
			PlanMaestroDetalle pmd = getPlanMaestroDetalle(pCSV.getId().getNumeroInicial(),
				pCSV.getId().getNumeroFinal());

			// Si el número ya existía en el plan maestro se actualizan los datos
			if (pmd != null) {
			    LOGGER.info("Plan Maestro Detalle en BD: " + pmd.toString());
			    if (!pmd.equals(pCSV)) {
				LOGGER.info("Actualizar noInicial: " + pmd.getId().getNumeroInicial() + " y noFinal: "
					+ pmd.getId().getNumeroFinal());
				...
				planMaestroDAO.update(pmd);
				listSyncronizados.add(pCSV);
			    }
			} else {
			    // Si el número no existe en el plan maestro se agrega
			    LOGGER.info("Se registrarán en el plan maestro: " + pCSV.getId().getNumeroInicial()
				    + " y noFinal: " + pCSV.getId().getNumeroFinal());
	    ...
	    LOGGER.info("Portaciones a procesar: " + cantidadRegistros + " cambios realizados: "
		    + listSyncronizados.size());
	...
	try {
		...
		if (pmd != null) {
		    ...
		    planMaestroDAO.delete(pmd);
		    listCancelados.add(pCSV);

		} else {
		    LOGGER.info("-No. encontro registros en BD con noInicial: " + pk.getNumeroInicial() + " y noFinal: "
			    + pk.getNumeroFinal());
		}
		line = br.readLine();
		i++;
	    }
	    LOGGER.info("Cancelados a procesar: " + i + " eliminaciones realizadas: " + listCancelados.size());
